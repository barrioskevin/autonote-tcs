...
const fillNotes = async (notes, todaysDate) => {
  ...
  let filled = 0;
  await Promise.all(
    studentPages.map(async ({ name, page }) => {
...
const run = async () => {
  ...
  if (argc !== 4 && argc !== 5) {
    ...
    console.log(
      "NOTE: Date should be provided 'mm/dd'. Omitting the date and only providing the file will try to submit notes for the current day.\n",
    );
    console.log(
      "Example of submitting a note for March 20th with a note file from a notes folder in the current directory.",
    );
